...
import { catalogs, type Page } from '../lib/site';
...
export function PageContent({ page }: { page: Page }) {
  ...
  const catalog = catalogs[page.path as keyof typeof catalogs] ?? [];
  const matches = (search: string, topic: string) => (!category || topic === category) && search.includes(query.trim().toLowerCase());
  ...
  const options: HTMLReactParserOptions = { replace(node) {
    ...
    if (node.name === 'img' && attrs.fetchpriority) {
      ...
      return <img {...attributesToProps(rest)} fetchPriority={fetchpriority as 'high' | 'low' | 'auto'}/>;
    }
    if ('data-contact-form' in attrs) return <ApplicationForm apply={page.path === '/apply'} />;
    if ('data-filter-form' in attrs) return <form className="filter-bar" role="search" onSubmit={e => e.preventDefault()} onReset={() => { setQuery(''); setCategory(''); }}>
    ...
    if ('data-library-item' in attrs) return <article {...attributesToProps(attrs)} hidden={!matches(attrs['data-search'], attrs['data-category'])}>{domToReact(node.children as DOMNode[], options)}</article>;
    if (node.name === 'a') return <SiteLink {...attributesToProps(attrs)}>{domToReact(node.children as DOMNode[], options)}</SiteLink>;
  } };
