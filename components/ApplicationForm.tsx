...
const initial: State = { success: false, message: '', errors: {} };
const empty: Values = { name: '', email: '', intent: '', city: '', organization: '', project: '', message: '', website: '' };
const builders = ['build', 'startup', 'developer'];
export function ApplicationForm({ apply }: { apply: boolean }) {
  const [values, setValues] = useState<Values>({ ...empty, intent: apply ? 'build' : '' });
  const [hydrated, setHydrated] = useState(false);
  const { search } = useLocation();
  const retry = useRef<{ fingerprint: string; key: string; created: number } | null>(null);
  ...
  useEffect(() => {
    ...
    if (intent && intents.some(([value]) => value === intent)) setValues(v => ({ ...v, intent }));
  }, [search]);
  const [state, action, pending] = useActionState(async (_previous: State, data: FormData): Promise<State> => {
    setOptimistic('Sending your message…');
    const payload = Object.fromEntries(data) as Values;
    ...
    if (Object.keys(errors).length) return { success: false, message: 'Please check the highlighted fields.', errors };
    const fingerprint = JSON.stringify(payload);
    if (retry.current?.fingerprint === fingerprint && Date.now() - retry.current.created >= 23 * 60 * 60 * 1000) return { success: false, errors: {}, message: 'This retry window has expired. Please contact advisor@studentspace.com to check whether your message was received before submitting again.' };
    if (!retry.current || retry.current.fingerprint !== fingerprint) retry.current = { fingerprint, key: crypto.randomUUID(), created: Date.now() };
    const controller = new AbortController();
    const timeout = setTimeout(() => controller.abort(), 15000);
    try {
      const response = await fetch('/api/apply', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': retry.current.key }, body: fingerprint, signal: controller.signal });
      const result = await response.json();
      if (response.ok && result.success === true) return { success: true, errors: {}, message: 'Your message has been submitted to StudentSpace. Thank you for getting in touch.' };
      return { success: false, message: typeof result.message === 'string' ? result.message : 'Unable to confirm submission. Your entries have been kept; please retry.', errors: result.errors && typeof result.errors === 'object' ? result.errors : {} };
    } catch { return { success: false, errors: {}, message: 'We could not confirm submission. Your entries have been kept. Retry the same message or email advisor@studentspace.com.' }; }
