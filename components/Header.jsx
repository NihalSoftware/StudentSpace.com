...
export function Header() {
	...
	const close = () => startTransition(() => {
		addTransitionType("menu");
		setOpen(false);
	});
    ...
    <header className={"site-header" + (enhanced ? " enhanced" : "")} onKeyDown={(event) => {
      ...
      <button ref={toggle} className="menu-toggle" hidden={!enhanced} aria-expanded={open} aria-controls="primary-navigation" onClick={() => startTransition(() => {
		addTransitionType("menu");
		setOpen(!open);
