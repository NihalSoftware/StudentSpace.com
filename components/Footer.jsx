...
export function Footer({ year }) {
  ...
  </div><div className="footer-contact"><span>Have a question?</span>{[
		company.givebackEmail,
		company.salesEmail,
		company.supportEmail
	].map((email) => <a key={email} href={"mailto:" + email}>{email}</a>)}</div>
    <div className="footer-bottom"><p>© {year} StudentSpace · A for-profit company with New Mexico roots.</p><nav aria-label="Policies">{[
		["/privacy", "Privacy"],
		["/terms", "Website terms"],
		["/licensing", "Licensing"],
		["/accessibility", "Accessibility"]
	].map(([href, label]) => <SiteLink key={href} href={href}>{label}</SiteLink>)}</nav></div>
  </div></footer>;
}
